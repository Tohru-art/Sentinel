...
from config import (COMPTIA_CERTS, CYBER_QUOTES, DISCORD_TOKEN, OPENAI_API_KEY,
...
async def setup_hook():
    ...
    if initialize_database():
        ...
        print(
            "⚠️ Database initialization failed, using fallback in-memory storage"
        )
...
@study_bot.tree.command(name="certs",
...
async def display_certifications(interaction: discord.Interaction):
    ...
    certifications_embed = discord.Embed(
        ...
        description=
        "*Professional certification paths for cybersecurity and IT excellence*",
        color=0x2B2D31)
    ...
    for cert_code, cert_details in COMPTIA_CERTS.items():
        ...
        certifications_embed.add_field(name=f"{cert_emoji} {cert_code}",
...
@study_bot.tree.command(
    name="selectcert",
    description="Select a CompTIA certification to study for")
...
async def select_certification_focus(interaction: discord.Interaction,
    ...
    # Study domains
    domains_formatted = '\n'.join(
        [f"• {domain}" for domain in selected_cert_info['domains']])
    selection_embed.add_field(name=f"{cert_emoji} Study Domains",
                              value=domains_formatted,
                              inline=False)

    # Next steps
    selection_embed.add_field(name="🚀 Ready to Begin",
...
async def display_study_statistics(interaction: discord.Interaction):
    ...
    if total_questions > 0:
        ...
        stats_embed.add_field(name="📊 Performance Overview",
                              value=f"Questions Answered: {total_questions}\n"
                              f"Correct Answers: {correct_answers}\n"
                              f"Study Score: {study_score}",
                              inline=True)

        stats_embed.add_field(name="📈 Overall Accuracy",
                              value=accuracy_bar,
                              inline=False)
    else:
    ...
    # Additional metrics
    stats_embed.add_field(name="🔥 Study Streak",
                          value=f"{study_streak} days",
                          inline=True)

    stats_embed.add_field(name="⏱️ Study Time",
                          value=f"{total_minutes} minutes",
                          inline=True)

    if user_discord_id in user_flashcard_collections and user_flashcard_collections[
            user_discord_id]:
        flashcard_count = len(user_flashcard_collections[user_discord_id])
        stats_embed.add_field(name="🗃️ Flashcards",
...
@study_bot.tree.command(
    name="practice",
    description="Generate AI-powered practice questions for your certification"
)
@app_commands.describe(difficulty="Choose question difficulty level",
...
async def generate_practice_questions(interaction: discord.Interaction,
    ...
    new_achievements = await check_achievements(user_discord_id,
                                                user_certification)
...
Generate questions that a CompTIA {user_certification} candidate would actually encounter on the real exam."""
        ...
        response = await openai_client.chat.completions.create(
            ...
            messages=[{
                ...
                "system",
                "content":
                f"You are a CompTIA {user_certification} subject matter expert with 15+ years of IT experience. You write realistic certification exam questions that mirror actual CompTIA exam style, difficulty, and technical accuracy. You understand real-world IT scenarios, current technologies, and industry best practices."
            }, {
                "role": "user",
                "content": ai_prompt
            }],
        ...
        if len(parsed_questions) != validated_count:
            raise ValueError(
                f"Expected {validated_count} questions, got {len(parsed_questions)}"
            )

        # Create success message
        print(
            f"🤖 Generating {validated_count} {difficulty} questions for {user_certification}"
        )
        await interaction.followup.send(
            f"🤖 Generated {validated_count} {difficulty} practice questions for {user_certification}! Get ready..."
        )
        ...
        first_message = await interaction.followup.send(
            embed=first_question_embed, view=first_question_view)
        ...
        print(
            f"✅ Generated {validated_count} questions for {interaction.user.name}"
        )
...
@study_bot.tree.command(
    name="leaderboard",
    description="View competitive study leaderboards and rankings")
async def show_leaderboard(interaction: discord.Interaction):
    ...
    try:
        ...
        leaderboard_embed = discord.Embed(
            ...
            description=
            "*Real-time performance rankings across all CompTIA certifications*",
            color=0x2B2D31)
        ...
        if daily_champs:
            ...
            daily_text += "```"
            leaderboard_embed.add_field(name="🏆 **Daily Champions**",
                                        value=daily_text,
                                        inline=True)
        else:
        ...
        if accuracy_masters:
            ...
            for i, master in enumerate(accuracy_masters, 1):
                color = "\u001b[32m" if master[
                    'accuracy'] >= 80 else "\u001b[33m" if master[
                        'accuracy'] >= 60 else "\u001b[31m"
                accuracy_text += f"{color}{i:2}.\u001b[0m   {master['username']:<15} {color}{master['accuracy']:>5.1f}%\u001b[0m\n"
            accuracy_text += "```"
            leaderboard_embed.add_field(name="🎯 **Accuracy Masters**",
                                        value=accuracy_text,
                                        inline=True)
        else:
            leaderboard_embed.add_field(
        ...
        if study_legends:
            ...
            leaderboard_embed.add_field(
                name="👑 **Study Legends**",
                value=
                "*The leaderboard awaits your excellence*\nStart your journey with `/practice`",
                inline=False)

        leaderboard_embed.set_footer(
            text="⚡ Live Rankings • Climb the leaderboard with /practice")
        ...
        error_embed = discord.Embed(
            ...
            description=
            "Sorry, I couldn't load the leaderboards right now. Please try again!",
            color=0xff4444)
...
@study_bot.tree.command(
    name="analysis",
    description="View your personalized AI-powered study analysis and insights"
)
async def show_study_analysis(interaction: discord.Interaction):
    ...
    try:
        ...
        analysis_embed = discord.Embed(
            ...
            description=
            "*AI-powered insights and personalized recommendations for exam success*",
            color=0x2B2D31)

        # Performance Overview
        if user_data['total_questions'] > 0:
            accuracy = (user_data['correct_answers'] /
                        user_data['total_questions']) * 100
            performance_bar = create_progress_bar(user_data['correct_answers'],
            ...
            analysis_embed.add_field(name="Overall Accuracy",
        ...
        if weak_spots:
            weak_data = [(spot['topic'], spot['accuracy'],
                          spot['questions_attempted']) for spot in weak_spots]
            weak_table = create_clean_stats_table(weak_data)

            analysis_embed.add_field(name="⚠️ Areas for Improvement",
                                     value=weak_table,
                                     inline=False)
        else:
            analysis_embed.add_field(
                ...
                value=
                "Answer more questions to identify weaknesses and get targeted insights!",
                inline=False)

        # Strengths Section
        if strengths:
            strength_data = [(strength['topic'], strength['accuracy'],
                              strength['questions_attempted'])
                             for strength in strengths]
            strength_table = create_clean_stats_table(strength_data)

            analysis_embed.add_field(name="✅ Your Strengths",
                                     value=strength_table,
                                     inline=False)

        # AI Recommendations
        recommendations = await generate_study_recommendations(
            user_discord_id, certification, weak_spots, strengths)
        if recommendations:
            analysis_embed.add_field(name="🤖 AI Study Recommendations",
                                     value=recommendations,
                                     inline=False)

        analysis_embed.set_footer(
            text=
            "Analysis powered by adaptive AI • Updates in real-time as you study"
        )
...
async def show_help_menu(interaction: discord.Interaction):
    ...
    # Study Management
    help_embed.add_field(name="📚 Study Management",
    ...
    # AI Tools
    help_embed.add_field(name="🧠 AI Study Tools",
                         value="`/practice` Practice questions\n"
    ...
    # Security Tools
    help_embed.add_field(name="🔒 Security Tools",
...
@study_bot.tree.command(
    name="about",
    description="View bot information, creator credits, and development details"
)
async def show_about_info(interaction: discord.Interaction):
    ...
    about_embed.add_field(
        name="👨‍💻 Developer",
        value=
        "**[Yorouki](https://github.com/Tohru-art)**\n*Open source project*",
        inline=True)
    ...
    about_embed.add_field(
        name="🔗 **Links**",
        value=
        "📂 **[GitHub Repository](https://github.com/Tohru-art/Sentinel)**\n"
...
@study_bot.tree.command(
    name="flashcards", description="Generate AI flashcards for CompTIA topics")
@app_commands.describe(topic="Specific topic to create flashcards for",
                       count="Number of flashcards (1-10)")
async def create_ai_flashcards(interaction: discord.Interaction,
                               topic: str = None,
                               count: int = 3):
...
Focus on important concepts students need to memorize for the exam."""
        ...
        response = await openai_client.chat.completions.create(
            ...
            messages=[{
                ...
                "system",
                "content":
                f"You are a CompTIA {certification} instructor creating study flashcards."
            }, {
                "role": "user",
                "content": prompt
            }],
        ...
        flashcards_embed = discord.Embed(
            ...
            description=
            f"*Generated {len(flashcards)} flashcards for {certification}*",
            color=0x2B2D31)
        ...
        flashcards_embed.set_footer(
            text=
            f"Total: {len(user_flashcard_collections[user_discord_id])} flashcards"
        )

        await interaction.followup.send(embed=flashcards_embed)
        print(
            f"🗃️ Generated {len(flashcards)} flashcards for {interaction.user.name} on {topic}"
        )

    except Exception as e:
        await interaction.followup.send(
            f"❌ Error generating flashcards: {str(e)}", ephemeral=True)
        print(f"❌ Flashcard generation error: {e}")


@study_bot.tree.command(
    name="explain",
    description="Get detailed AI explanations of any study topic")
...
async def explain_topic(interaction: discord.Interaction, topic: str):
    ...
    if not openai_client:
        await interaction.response.send_message(
            "❌ AI features are currently unavailable.", ephemeral=True)
        return
...
Make it educational and exam-focused."""
        ...
        response = await openai_client.chat.completions.create(
            ...
            messages=[{
                ...
                "system",
                "content":
                f"You are an expert CompTIA {certification} instructor."
            }, {
                "role": "user",
                "content": prompt
            }],
        ...
        explain_embed.set_footer(
            text=f"Powered by AI • {certification} focused")
        ...
        await interaction.followup.send(
            f"❌ Error generating explanation: {str(e)}", ephemeral=True)
...
@study_bot.tree.command(
    name="pomodoro",
    description="Start a focused study session with Pomodoro timer")
...
async def start_pomodoro_session(interaction: discord.Interaction,
    ...
    durations = {"study": 25, "short_break": 5, "long_break": 15}
    ...
    session_embed = discord.Embed(
        title=
        f"{session_emoji} Pomodoro {session_type.replace('_', ' ').title()}",
        description=f"*{duration_minutes}-minute focused session active*",
        color=0x2B2D31)

    session_embed.add_field(name="⏱️ Session Info",
    ...
    if session_type == "study":
        session_embed.add_field(name="💡 Study Tips",
                                value="• Focus on CompTIA materials\n"
                                "• Use `/practice` actively\n"
                                "• Note weak areas",
                                inline=True)
    else:
        session_embed.add_field(name="🧘 Break Tips",
    ...
    study_bot.loop.create_task(
        pomodoro_completion_task(interaction, user_discord_id, session_type,
                                 duration_minutes))


async def pomodoro_completion_task(interaction, user_id, session_type,
                                   duration_minutes):
    ...
    if user_id in pomodoro_sessions:
        ...
        completion_embed = discord.Embed(
            ...
            description=
            f"Your {session_type.replace('_', ' ')} session ({duration_minutes} min) is finished!",
            color=0x00ff00)
        ...
        if session_type == "study":
            completion_embed.add_field(
                name="Great Job!",
                value=
                "Time for a break! Use `/pomodoro short_break` to start a 5-minute break.",
                inline=False)
            ...
            completion_embed.add_field(
                name="Break Complete!",
                value=
                "Ready to get back to studying? Use `/pomodoro study` for another focused session.",
                inline=False)
        ...
        try:
            ...
            try:
                await interaction.channel.send(
                    f"🎉 <@{user_id}> Your Pomodoro session is complete!",
                    embed=completion_embed)
            except:
                pass


@study_bot.tree.command(name="stoppomodoro",
                        description="Stop your current Pomodoro session")
async def stop_pomodoro_session(interaction: discord.Interaction):
    ...
    if user_discord_id not in pomodoro_sessions:
        await interaction.response.send_message(
            "❌ You don't have an active Pomodoro session!", ephemeral=True)
        return
    ...
    stop_embed = discord.Embed(
        ...
        description=
        f"Your {session['type'].replace('_', ' ')} session has been ended early.",
        color=0xff6b6b)
    ...
    print(
        f"⏹️ {interaction.user.name} stopped Pomodoro session after {elapsed_minutes} minutes"
    )
...
@study_bot.tree.command(
    name="cyberquote",
    description="Get inspirational cybersecurity quotes for motivation")
async def send_cybersecurity_quote(interaction: discord.Interaction):
    ...
    quote_embed = discord.Embed(
        ...
        description=
        f"*Professional insights from industry leaders*\n\n**\"{quote}\"**",
        color=0x2B2D31)

    quote_embed.set_footer(
        text="🔒 Stay motivated • Security is a mindset, not a destination")
...
@study_bot.tree.command(
    name="scan",
    description="Scan a host and port for network security analysis")
@app_commands.describe(host="Target host/IP address to scan",
                       port="Port number to check")
async def network_port_scan(interaction: discord.Interaction, host: str,
                            port: int):
    ...
    try:
        ...
        if not (1 <= port <= 65535):
            await interaction.followup.send(
                "❌ Port must be between 1 and 65535!", ephemeral=True)
            return
        ...
        scan_embed = discord.Embed(
            ...
            description=
            f"*TCP connection test results for security assessment*",
            color=0x2B2D31)
        ...
        scan_embed.set_footer(
            text="⚠️ Ethical Use Only • Only scan authorized targets")
...
@study_bot.tree.command(name="hash",
...
async def generate_hash(interaction: discord.Interaction, text: str):
    ...
    try:
        ...
        hash_embed.add_field(name="📝 **Input Text**",
                             value=f"```\n{input_preview}\n```",
                             inline=False)
        hash_embed.add_field(name="🔐 **SHA-256 Hash**",
                             value=f"```\n{hex_hash}\n```",
                             inline=False)

        hash_embed.set_footer(
            text="🔒 Secure • Industry-standard SHA-256 algorithm")
        ...
        await interaction.response.send_message(
            f"❌ Hash generation failed: {str(e)}", ephemeral=True)
        print(f"❌ Hash generation error: {e}")


@study_bot.tree.command(
    name="iplookup",
    description="Look up geographic and ISP information for an IP address")
...
async def lookup_ip_info(interaction: discord.Interaction, ip: str):
    ...
    try:
        ...
        if 'error' in data:
            await interaction.followup.send(
                f"❌ IP lookup failed: {data['reason']}", ephemeral=True)
            return
        ...
        ip_embed.add_field(name="🌍 **Geographic Location**",
                           value=location,
                           inline=True)
        ...
        ip_embed.add_field(name="🌐 **Network Information**",
                           value=network_info,
                           inline=True)

        ip_embed.set_footer(
            text="📍 Powered by ipapi.co • Educational use only")

        await interaction.followup.send(embed=ip_embed)
        print(f"🌍 IP lookup for {ip} by {interaction.user.name}")

    except Exception as e:
        await interaction.followup.send(f"❌ IP lookup failed: {str(e)}",
                                        ephemeral=True)
...
@study_bot.tree.command(
    name="passwordcheck",
    description="Analyze password strength and time to crack scenarios")
@app_commands.describe(
    password="Password to analyze (hidden from other users)")
async def analyze_password_strength(interaction: discord.Interaction,
                                    password: str):
    ...
    def calculate_entropy(pwd):
        ...
        char_sets = [(string.ascii_lowercase, "lowercase letters"),
    ...
    def get_strength_info(entropy):
        ...
        if entropy <= 0:
            ...
            return min(100,
                       int(75 + ((entropy - 70) / 20) * 25)), "Strong", "🟢"
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=
            "*Comprehensive security analysis with crack-time scenarios*",
            color=0x2B2D31)
        ...
        embed.add_field(name="⏱️ **Time to Crack Scenarios**",
                        value=scenario_text,
                        inline=False)
        ...
        embed.add_field(name="📊 **Character Analysis**",
                        value=char_info,
                        inline=True)
        ...
        embed.add_field(name="🔍 **Security Issues**",
                        value=weakness_text,
                        inline=True)

        embed.set_footer(
            text="🔐 Password analysis complete • Keep your passwords private")
        ...
        await interaction.followup.send(
            "❌ Password analysis failed. Please try again.", ephemeral=True)
